    ...
    <footer className="border-t border-white/[0.06] bg-[#06060c]">
      <div className="mx-auto max-w-7xl px-6 md:px-10 h-11 flex items-center justify-between gap-4">
        ...
        <Link href="/" className="flex items-center shrink-0">
          ...
          <span className="font-bold tracking-[0.22em] uppercase text-zinc-700 text-[11px] hidden sm:block" style={{ marginLeft: "-2px" }}>VENUES · USC</span>
        </Link>
        ...
        <nav className="hidden lg:flex items-center gap-4">
          {navLinks.map(({ href, label }) => (
            <Link
              key={href}
              href={href}
              className="text-[10px] text-zinc-700 hover:text-zinc-400 uppercase tracking-[0.12em] transition-colors duration-200"
            >
              {label}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-3 shrink-0">
          <span className="text-[10px] text-zinc-700 hidden md:block">
