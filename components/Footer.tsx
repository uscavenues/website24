...
export default function Footer() {
  return (
    <footer className="border-t border-white/[0.06] bg-[#06060c]">
      <div className="mx-auto max-w-7xl px-6 sm:px-10 py-4 flex flex-wrap items-center justify-between gap-x-8 gap-y-3">
        ...
        <nav className="hidden sm:flex items-center gap-5">
          {NAV.map(({ href, label }) => (
            <Link key={href} href={href} className="text-[10px] uppercase tracking-[0.12em] text-zinc-600 hover:text-zinc-300 transition-colors duration-200">
              {label}
            </Link>
          ))}
        </nav>
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="text-[10px] text-zinc-700 tabular-nums" suppressHydrationWarning>
            © {new Date().getFullYear()} Avenues · USC
          </span>
        </div>
