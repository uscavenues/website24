...
function PasswordGate({ onUnlock }: { onUnlock: () => void }) {
  ...
  const handleAction = (formData: FormData) => {
    const value = (formData.get("password") as string ?? "").toLowerCase().trim();
    if (value === PASSWORD) {
    ...
    <div className="min-h-screen bg-black flex flex-col items-center justify-center">
      ...
      >
        <div className="relative w-10 h-10 opacity-50">
          <Image src="/assets/icons/avenues-logo.png" alt="Avenues" fill sizes="40px" className="object-contain invert" />
        </div>
        <p className="text-[10px] uppercase tracking-[0.5em] text-zinc-600">Password?</p>
        <form action={handleAction} className="flex flex-col items-center gap-3">
          <input
            type="password"
            name="password"
            value={input}
